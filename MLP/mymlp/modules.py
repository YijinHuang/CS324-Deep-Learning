...
class Linear(object):
    def __init__(self, in_features, out_features):
        ...
        """
        params = {}
        params['weight'] = np.random.randn(in_features, out_features)*np.sqrt(2/in_features)
...
class Optimizer:
    def __init__(self, model, learning_rate, optimizer=''):
        ...
        if optimizer == 'momentum':
            ...
            self.update = self.optimize_RMSProp
        else:
            self.update = self.basic_optimize

    def optimize(self):
        weight_variables = {idx: {'v': 0} for idx in range(len(self.trainable_layer))}
        bias_variables = {idx: {'v': 0} for idx in range(len(self.trainable_layer))}
        for idx in range(len(self.trainable_layer)):
            layer = self.trainable_layer[idx]
            weight_variable = weight_variables[idx]
            bias_variable = bias_variables[idx]
            layer.params['weight'] -= self.update(layer.grads['weight'], weight_variable)
            layer.params['bias'] -= self.update(layer.grads['bias'], bias_variable)
    ...
    def optimize_RMSProp(self, grad, variables, beta=0.9, e=1e-8):
